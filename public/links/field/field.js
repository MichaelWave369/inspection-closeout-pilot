(() => {
  ...
  const journeySequence = byId('journey-sequence')
  ...
  function selectJourney(id) {
    ...
    journeySequence.replaceChildren()
    ...
    selectedJourney.nodes.forEach((nodeId, index) => {
      ...
      const link = el('a', { href: projectUrl(nodeId), text: `${index + 1}. ${project?.title || nodeId}` })
      sequence.append(link)
    })
    journeyDetail.append(copy, sequence)
    journeySequence.replaceWith(sequence)
    applyMapState()
  ...
  function chooseProofAction(actionId) {
    ...
    if (actionId === 'advance') {
      completedThrough = Math.max(completedThrough, selectedProofStage)
      const next = Math.min(selectedProofStage + 1, proof.stages.length - 1)
      if (next !== selectedProofStage) selectedProofStage = next
    }
  ...
  initialize().catch(error => {
    ...
    const fatal = el('div', { className: 'fatal', text: `The guided layer could not load its canonical Archive records: ${error.message}` })
    journeyGrid.replaceChildren(fatal)
  })
})()
